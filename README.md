...
服务端设 `MCP_API_KEYS` 环境变量即可支持多用户：

```bash
MCP_API_KEYS='{"key-a":"userId-a","key-b":"userId-b"}'
```

用户在自己的 AI 客户端配置各自 key，数据完全隔离。

### 更新 MCP 包

```bash
pnpm --filter @todograph/mcp publish:npm   # 手动发布
# 或 push to main 触发 GitHub Actions 自动发布
```
...
- [x] **AI Agent (MCP)**：10 个 MCP 工具，AI 管理任务依赖、自动布局、批量创建
