...
export function resolveListDropIntent(input: DropIntentInput): ListDropIntent {
  ...
  if (target && targetRect && target.id !== dragged.id && !isDescendant(input.byId, target.id, dragged.id)) {
  ...
  if (dragged.parentId && input.clientX - input.startX <= -48) return { kind: 'unparent' };
  return { kind: 'none' };
}
