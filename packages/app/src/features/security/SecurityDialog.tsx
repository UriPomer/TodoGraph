    ...
    <div className="fixed inset-0 z-[210] flex items-center justify-center p-4">
      ...
      <div className="relative max-h-full w-full max-w-lg overflow-y-auto rounded-xl border border-border bg-card p-5 shadow-2xl">
        <header className="mb-4 flex items-center justify-between">
          <h2 className="flex items-center gap-2 text-sm font-semibold"><Shield className="h-5 w-5 text-[hsl(var(--primary))]" />账号与数据安全</h2>
        ...
        <div className="space-y-4">
          <section className="rounded-lg border border-border p-3">
            <h3 className="mb-2 text-xs font-semibold">修改密码</h3>
            <div className="space-y-2">
              ...
              <Button size="sm" onClick={changePassword} disabled={!passwords.current || !passwords.next || !passwords.confirm || mismatch || busy === 'password'}>
                {busy === 'password' && <Loader2 className="mr-1 h-3.5 w-3.5 animate-spin" />}更新密码
              </Button>
            </div>
          </section>
          <section className="rounded-lg border border-border p-3">
            <h3 className="mb-2 text-xs font-semibold">完整数据备份</h3>
            <div className="flex gap-2">
              <Button size="sm" variant="secondary" onClick={exportJson} disabled={busy === 'export'}><Download className="mr-1 h-3.5 w-3.5" />导出 JSON</Button>
              <label className="inline-flex h-8 cursor-pointer items-center rounded-md border border-input px-3 text-xs font-medium hover:bg-accent"><Upload className="mr-1 h-3.5 w-3.5" />导入 JSON
          ...
          <section className="rounded-lg border border-border p-3">
            ...
            {!activePageId || backups.length === 0 ? <p className="text-xs text-muted-foreground">{busy === 'backups' ? '加载中...' : activePageId ? '暂无自动备份' : '当前没有已加载页面'}</p> : (
              <div className="space-y-2">
                ...
                <Button size="sm" variant="secondary" onClick={restore} disabled={!selectedBackup || busy === 'restore'}><ArchiveRestore className="mr-1 h-3.5 w-3.5" />恢复所选备份</Button>
              </div>
            )}
          </section>
