...
export interface WorkspaceMovePlan {
  source: PageData;
  target: PageData;
  result: MoveNodesResponse;
}

/** Pure cross-page move planner shared by ordinary moves and transactional page merges. */
export function planWorkspaceMove(
  source: PageData,
  target: PageData,
  userSelected: readonly string[],
  targetSupportsDependencies: boolean,
): WorkspaceMovePlan {
  ...
  const nextTarget: PageData = {
    nodes: [...safeTargetNodes, ...placeMovedNodesOnTarget(safeTargetNodes, movedNodes)],
    edges: [...target.edges, ...movedEdges],
  };
  if (!isDAG(nextSource) || !isDAG(nextTarget)) throw new Error('resulting page would contain a cycle');
